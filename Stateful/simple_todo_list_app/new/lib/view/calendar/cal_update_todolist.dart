...
class CalUpdateTodolist extends StatefulWidget {
  final DatabaseHandle db;
  final TodoList todo;
  const CalUpdateTodolist({super.key, required this.db, required this.todo});

  @override
  State<CalUpdateTodolist> createState() => _CalUpdateTodolistState();
}

class _CalUpdateTodolistState extends State<CalUpdateTodolist> {
  // Property
  ...
  void initState() {
    ...
    isAlarm = widget.todo.isAlarm == 0 ? false : true;
    startDate = widget.todo.startDate!;
    contentController.text = widget.todo.content != null ? widget.todo.content! : '';
    duration = Duration(hours: widget.todo.startDate!.hour, minutes: widget.todo.startDate!.minute);
  }
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text('업데이트', style: TextStyle(fontSize: 20)),
          ),
          Padding(
  ...
  beforeAction() async {
    if (titleController.text.trim().isNotEmpty) {
      ...
      TodoList todolist = TodoList(
        ...
        startDate: DateTime(startDate.year, startDate.month, startDate.day, hours, mins),
        content: contentController.text.trim(),
        importance: dropDownValue,
        isAlarm: isAlarm ? 1 : 0,
      );

      await updateAction(todolist);
    } else {
  ...
  updateAction(TodoList todolist) async {
    int result = await widget.db.updateTodoList(todolist, TableName.todolist);
